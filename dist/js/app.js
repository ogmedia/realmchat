...
var RC_VIEWS = (function(window, $, Backbone, _){

	var gameInfoView = Backbone.View.extend({
		el: '#header',
		events: {
		//	'click #twitter-login': rc_auth.authTwitter,
		//	'click #facebook-login': rc_auth.authFacebook, 
		},
		initialize: function(){
			this.render();
		},
...
var RC_AUTH = (function(window, views){
			...
			playersRef.authWithOAuthPopup("twitter", function(error, auth) {
			  if (error) {
			    console.warn("Login Failed!", error);
			   	...
			   	set_profile_data(auth.twitter, authData.uid);
			  }
			});
		},
...
'use strict';
var RC_APP = (function(THREE,RC_APP){
...
'use strict';
var RC_VC = (function(window,$, RC_APP){
	// window.VC = function(){
	// 	var ready = function(){
